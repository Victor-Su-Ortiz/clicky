...
- API keys for: [MiniMax](https://platform.minimax.io), [AssemblyAI](https://www.assemblyai.com)
...
For the MiniMax voice ID, open `wrangler.toml` and set it there (it's not sensitive). Any built-in MiniMax voice id works, e.g. `English_FriendlyPerson`:

```toml
[vars]
MINIMAX_VOICE_ID = "English_FriendlyPerson"
```
...
You'll find it in:
- `CompanionManager.swift` — MiniMax chat + MiniMax TTS
...
**Menu bar app** (no dock icon) with two `NSPanel` windows — one for the control panel dropdown, one for the full-screen transparent cursor overlay. Push-to-talk streams audio over a websocket to AssemblyAI, sends the transcript + screenshot to MiniMax-M3 via streaming SSE (through MiniMax's Anthropic-compatible API), and plays the response through MiniMax TTS. The model can embed `[POINT:x,y:label:screenN]` tags in its responses to make the cursor fly to specific UI elements across multiple monitors. All API calls are proxied through a Cloudflare Worker.
...
leanring-buddy/          # Swift source (yes, the typo stays)
  ...
  MiniMaxAPI.swift          # MiniMax streaming client
  MiniMaxTTSClient.swift    # Text-to-speech playback
